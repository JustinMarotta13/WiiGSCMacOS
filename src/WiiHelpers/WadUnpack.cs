...
namespace Wii
{
    public class WadUnpack
    {
        ...
        {
            if (!destination.EndsWith(".app")) destination = Path.Combine(destination, "00000000.app");
        ...
        /// <summary>
        /// Unpacks the the wad file to *wadpath*\wadunpack\
        /// </summary>
        /// <param name="pathtowad"></param>
        public static string UnpackWad(string pathtowad)
        {
            string destinationpath = Path.GetDirectoryName(pathtowad);
            byte[] wadfile = Tools.LoadFileToByteArray(pathtowad);
            return UnpackWad(wadfile, destinationpath);
        }
        ...
        public static string UnpackWad(byte[] wadfile, string destinationpath, out bool hashesmatch)
        {
            ...
            //unpack cert
            using (FileStream cert = new FileStream(Path.Combine(destinationpath, wadpath + ".cert"), FileMode.Create))
            {
            ...
            //unpack ticket
            using (FileStream tik = new FileStream(Path.Combine(destinationpath, wadpath + ".tik"), FileMode.Create))
            {
                tik.Seek(0, SeekOrigin.Begin);
                tik.Write(wadfile, tikpos, tiksize);
            }

            string tmdPath = Path.Combine(destinationpath, wadpath + ".tmd");
            //unpack tmd
            ...
            try
            {
                if (footersize > 0)
                {
                    using (FileStream trailer = new FileStream(Path.Combine(destinationpath, wadpath + ".trailer"), FileMode.Create))
                    {
            ...
            for (int i = 0; i < contents.GetLength(0); i++)
            {
                ...
                FileStream content = new FileStream(Path.Combine(destinationpath, contents[i, 1] + ".app"), FileMode.Create);
