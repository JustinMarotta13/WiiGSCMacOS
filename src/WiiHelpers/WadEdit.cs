...
namespace Wii
{
    public class WadEdit
    {
        ...
        public static void UpdateTmdContents(string tmdfile)
        {
            ...
            string directory = Path.GetDirectoryName(tmdfile);
            ...
            for (int i = 0; i < contentcount; i++)
            {
                ...
                string fileName = id;
                string filePath = Path.Combine(directory, fileName + ".app");

                if (!File.Exists(filePath))
                {
                    fileName = index;
                    filePath = Path.Combine(directory, fileName + ".app");
                }

                if (File.Exists(filePath))
                {
                    byte[] content = Wii.Tools.LoadFileToByteArray(filePath);
