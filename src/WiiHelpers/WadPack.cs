...
namespace Wii
{
    ...
    public class WadPack
    {
        ...
        {
            if (contentdirectory[contentdirectory.Length - 1] != Path.DirectorySeparatorChar) { contentdirectory = contentdirectory + Path.DirectorySeparatorChar; }
        ...
        public static void PackWad(string contentdirectory, string destinationfile, byte[] newTitleId)
        {
            ...
            /***CHANGE TITLE ID****/
            // If newTitleId is 8 bytes, update both upper and lower Title ID
            // If only 4 bytes, update lower Title ID only (legacy behavior)
            if (newTitleId.Length >= 8)
            {
                // Full 8-byte Title ID provided
                // Update ticket (Title ID at 0x1DC-0x1E3)
                tik[0x1dc] = newTitleId[0]; // Upper
                tik[0x1dd] = newTitleId[1];
                tik[0x1de] = newTitleId[2];
                tik[0x1df] = newTitleId[3];
                tik[0x1e0] = newTitleId[4]; // Lower
                tik[0x1e1] = newTitleId[5];
                tik[0x1e2] = newTitleId[6];
                tik[0x1e3] = newTitleId[7];

                // Update TMD (Title ID at 0x18C-0x193)
                tmd[0x18c] = newTitleId[0]; // Upper
                tmd[0x18d] = newTitleId[1];
                tmd[0x18e] = newTitleId[2];
                tmd[0x18f] = newTitleId[3];
                tmd[0x190] = newTitleId[4]; // Lower
                tmd[0x191] = newTitleId[5];
                tmd[0x192] = newTitleId[6];
                tmd[0x193] = newTitleId[7];
            }
            else
            {
                // Legacy: Only 4 bytes provided, update lower Title ID only
            ...
            for (int i = 0; i < contents.GetLength(0); i++)
            {
                ...
                byte[] thiscont = Tools.LoadFileToByteArray(Path.Combine(contentdirectory, contents[i, 1] + ".app"));
        ...
        {
            if (nandpath[nandpath.Length - 1] != Path.DirectorySeparatorChar) { nandpath = nandpath + Path.DirectorySeparatorChar; }
            string path1 = path.Remove(8);
            string path2 = path.Remove(0, 9);
            string ticketdir = Path.Combine(nandpath, "ticket", path1);
            string contentdir = Path.Combine(nandpath, "title", path1, path2, "content");
            string sharedir = Path.Combine(nandpath, "shared1");
            string certdir = Path.Combine(nandpath, "sys");
            ...
            if (!File.Exists(Path.Combine(certdir, "cert.sys"))) throw new FileNotFoundException("File doesn't exist:\r\n" + Path.Combine(certdir, "cert.sys"));

            byte[] cert = Tools.LoadFileToByteArray(Path.Combine(certdir, "cert.sys"));
            byte[] tik = Tools.LoadFileToByteArray(Path.Combine(ticketdir, path2 + ".tik"));
            byte[] tmd = Tools.LoadFileToByteArray(Path.Combine(contentdir, "title.tmd"));
            ...
            Tools.ChangeProgress(0);
            byte[] titlekey = WadInfo.GetTitleKey(tik);
            byte[] contentmap = Tools.LoadFileToByteArray(Path.Combine(sharedir, "content.map"));
            ...
            for (int i = 0; i < contents.GetLength(0); i++)
            {
                ...
                if (contents[i, 2] == "8001")
                {
                    ...
                    thiscont = Tools.LoadFileToByteArray(Path.Combine(sharedir, contname + ".app"));
                }
                else thiscont = Tools.LoadFileToByteArray(Path.Combine(contentdir, contents[i, 0] + ".app"));
