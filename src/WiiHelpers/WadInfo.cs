...
namespace Wii
{
    public class WadInfo
    {
        ...
        {
            if (File.Exists(Path.Combine(AppContext.BaseDirectory, "common-key.bin")) || File.Exists(Path.Combine(AppContext.BaseDirectory, "key.bin")))
            {
        ...
        public static string GetNandPath(byte[] wadtiktmd, int type)
        {
            ...
            thispath = thispath.Insert(8, Path.DirectorySeparatorChar.ToString());
            return thispath;
        }
        ...
        {
            byte[] commonkey;
            string keyPath = Path.Combine(AppContext.BaseDirectory, "shared", "common-key");
            
            if (File.Exists(keyPath))
            {
                commonkey = Tools.LoadFileToByteArray(keyPath);
            }
            else
            {
                // Use default Wii common key if file not present
                commonkey = new byte[] { 0xeb, 0xe4, 0x2a, 0x22, 0x5e, 0x85, 0x93, 0xe4, 0x48, 0xd9, 0xc5, 0x45, 0x73, 0x81, 0xaa, 0xf7 };
            }
